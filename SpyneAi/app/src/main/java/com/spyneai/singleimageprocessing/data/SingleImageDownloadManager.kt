...
class SingleImageDownloadManager(var task : SingleDownloadTask, var listener : Listener) {
    ...
    interface Listener {
        fun onSuccess(task: SingleDownloadTask)
        fun onScan(filePath : String)
        fun onRefresh(filePath : String)
        fun onFailure(task: SingleDownloadTask)
    }
}