        ...
        do {
            ...
            if (connectionLost){
                ...
                if (projectWithSku == null){
                    ...
                    listener.inProgress("Creating project ${projectWithSku.projectApp?.projectName} and its sku's",ServerSyncTypes.CREATE)
                    isActive = true

                    val properties = HashMap<String,Any?>().apply {
                        put("project_id",projectWithSku.projectApp?.uuid)
                    ...
                    var vehicleDataList: MutableList<ProjectBody.VehicleData> = mutableListOf()
                    var vehicleData: ProjectBody.VehicleData? = null

                    projectWithSku.let {
                        it.projectApp?.let { project ->
                                    ...
                                    if (!sku.isCreated && sku.isSelectAble) {
                                        skuList.add(
                                            ProjectBody.SkuData(
                                                ...
                                                localId = sku.uuid,
                                                skuName = sku.skuName!!,
                                                prodCatId = sku.categoryId.toString(),
                            ...
                            projectBody = ProjectBody(
                                    ...
                                    categoryId = project.categoryId.orEmpty(),
...
//                val s = SpyneSpyneAppDatabase.getInstance(BaseApplication.getContext()).shootDao().updateVideoSkuAndProjectIds(projectId,skus.skuId,skus.localId)
