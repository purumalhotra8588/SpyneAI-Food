...
class DraftGridFragment : BaseFragment<ShootViewModelApp, FragmentGridEcomBinding>(),
    ...
    private fun setDraftImages() {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            val list = viewModel.skuApp?.uuid?.let { viewModel.getImagesbySkuId(it) }
            ...
            list?.forEachIndexed { index, image ->
                val shootData = ShootData(
                    ...
                    angle = image.angle!!,
                    overlayId = image.overlayId?.toInt()!!,
                    sequence = index.plus(1)
                )
